...
export const CrudTarjetas = () => {
  ...
  const filtrar = (terminoBusqueda:string) =>{
    let resultadosBusqueda = dataFiltrada.filter((elemento:tipoObjeto) =>{
      if (
        elemento.ID_TARJETA.toString().includes(terminoBusqueda)
      ){
        return elemento;
      }
    })
  ...
  const peticionPut=async()=>{
    ...
    .then(response=>{
      ...
      dataNueva.map((tarjeta:tipoObjeto)=>{
        if(tarjeta.ID_TARJETA===seleccionado.ID_TARJETA){
          tarjeta.ID_CLIENTE = seleccionado.ID_CLIENTE;
          tarjeta.BALANCE = seleccionado.BALANCE;
          tarjeta.FECHA_CREACION = seleccionado.FECHA_CREACION;
          tarjeta.ESTATUS = seleccionado.ESTATUS;
        }
      });
      setData(dataNueva);
  ...
  const peticionDelete=async()=>{
    ...
    .then(response=>{
      setData(data.filter((tarjeta:tipoObjeto)=>tarjeta.ID_TARJETA !== seleccionado.ID_TARJETA));
  ...
  const seleccionarCliente=(tarjeta:any, caso:string)=>{
  ...
  const bodyEditar = (
    <div className={styles.modal}>
      <h3>Editar tarjeta {seleccionado && seleccionado.ID_TARJETA} </h3>
      <TextField
        className={styles.inputMaterial}
        label="ID CLIENTE"
        name="ID_CLIENTE"
  ...
  const bodyEliminar=(
    <div className={styles.modal}>
      <p>Estás seguro que deseas eliminar a la tarjeta <b>{seleccionado && seleccionado.ID_CLIENTE}</b>? </p>
      <div>
